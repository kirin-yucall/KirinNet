...
> **Version:** 0.2（修订草案）
> **Date:** 2026-08-27
> **Status:** **KNET-CC-015 草案·待会签，未会签单边定稿无效**（v0.1 曾于 2026-08-08 经 KNET-CC-007 会签定稿；本 v0.2 为 §5.1×§5.3 多跳签名互斥缺陷澄清修订）
...
| v0.2 修订（KNET-CC-015 草案·待会签，未会签单边定稿无效） | 澄清 §5.1×§5.3 多跳签名互斥缺陷：发起方签名覆盖**签发前缀**（验签输入 = `forward_chain` 首 `[src_domain]`），任意跳可验；防环仍用全链；hop 级逐跳签名列 v2 占位不定型。正文其余条款零改动 |
