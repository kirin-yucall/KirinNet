...
> **Version:** 0.3（修订草案）
> **Date:** 2026-08-29
> **Status:** v0.2 **已会签定稿（KNET-CC-015，2026-08-28 节点 PM 会签通过 + 协议 PM 合并 master 3c45a68 联合关闭）**（v0.1 曾于 2026-08-08 经 KNET-CC-007 会签定稿；本 v0.2 为 §5.1×§5.3 多跳签名互斥缺陷澄清修订）；**v0.3（§5.3-4 answer 例外澄清）为 KNET-CC-019 修订草案·待会签，未会签单边定稿无效**
...
| v0.3 修订（KNET-CC-019 修订草案·待会签，未会签单边定稿无效，2026-08-29） | §5.3-4 前缀完整性 MUST **追加**一句显式例外：`answer` 类型不适用本条（`answer` 信封 `src_domain`=应答方、`forward_chain` 保留查询链、链首=查询发起方≠应答方，链语义见 §5.1；其链完整性由 answer 签名输入的**全链 JCS** 承载，§5.3-6）；既有 MUST 文字零删改，正文其余条款零改动。背景：波 2 对照表 CC-6 后半附注——节点已按唯一语义自洽读法实现（`flood_engine.js` `parseEnvelope` 对非 answer 包执行 `chain[0]==src_domain` 校验、answer 按全链 JCS 验签）且 116/116→138/138 测试在案 |
...
4. **前缀完整性约束（MUST）:** 接收方校验 `forward_chain[0] == src_domain`，不满足即拒收（fail-closed）——签发前缀锚定发起方身份，首元素被篡改则验签必败，双重保证。**例外（answer 类型，KNET-CC-019 修订草案·待会签）:** 前述 MUST 不适用于 `answer` 类型——`answer` 信封 `src_domain` 为应答方、`forward_chain` 保留查询链（链首 = 查询发起方 ≠ 应答方，链语义见 §5.1），按字面执行本条将拒收全部合法多跳 answer；`answer` 的链完整性由 answer 签名输入的**全链 JCS** 承载（§5.3-6，JCS 规范化按 §2.2）。
...
| v0.3 | 2026-08-29 | **KNET-CC-019 修订草案·待会签（未会签单边定稿无效）。** §5.3-4 前缀完整性 MUST 追加一句显式 answer 例外：`answer` 信封 `src_domain`=应答方、`forward_chain` 保留查询链（链首=查询发起方≠应答方，链语义见 §5.1），按字面执行 MUST 将拒收全部合法多跳 answer；`answer` 的链完整性由 answer 签名输入的**全链 JCS** 承载（§5.3-6，JCS 规范化按 §2.2）。既有 MUST 文字零删改，正文其余条款零改动；头部 Version 0.2→0.3（修订草案）/Date/Status 草案子句 + §0 修订行 + 页脚状态戳同步。背景：波 2 对照表 CC-6 后半附注——节点已按唯一语义自洽读法实现（`flood_engine.js` `parseEnvelope` 仅对非 answer 包执行 `chain[0]==src_domain` 校验、answer 验签输入取全链 JCS）且 116/116→138/138 测试在案 |

---

*本文件为 T1 泛洪协议 v0.3（v0.2 已会签定稿：KNET-CC-007〔v0.1，2026-08-08〕+ KNET-CC-015〔v0.2，2026-08-28 节点 PM 会签通过 + 合并 master 3c45a68 联合关闭〕；**v0.3 为 KNET-CC-019 修订草案·待会签，未会签单边定稿无效**），是 T3/T4/T6/T8 的共同承载层。签名覆盖范围引用 T9（`DECISIONS.md` §9.3.2）不重定义；多跳语境下 `forward_chain` 验签输入取签发前缀（§5.3；`answer` 类型例外见 §5.3-4，v0.3 修订草案·待会签）。*
