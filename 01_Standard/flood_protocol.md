...
> **Version:** 0.2（修订草案）
> **Date:** 2026-08-27
> **Status:** **已会签定稿（KNET-CC-015，2026-08-28 节点 PM 会签通过 + 协议 PM 合并 master 3c45a68 联合关闭）**（v0.1 曾于 2026-08-08 经 KNET-CC-007 会签定稿；本 v0.2 为 §5.1×§5.3 多跳签名互斥缺陷澄清修订）
...
4. **前缀完整性约束（MUST）:** 接收方校验 `forward_chain[0] == src_domain`，不满足即拒收（fail-closed）——签发前缀锚定发起方身份，首元素被篡改则验签必败，双重保证。
...
*本文件为 T1 泛洪协议 v0.2（已会签定稿：KNET-CC-007〔v0.1，2026-08-08〕+ KNET-CC-015〔v0.2，2026-08-28 节点 PM 会签通过 + 合并 master 3c45a68 联合关闭〕），是 T3/T4/T6/T8 的共同承载层。签名覆盖范围引用 T9（`DECISIONS.md` §9.3.2）不重定义；多跳语境下 `forward_chain` 验签输入取签发前缀（§5.3）。*
