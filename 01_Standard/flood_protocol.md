...
| v0.4 修订（KNET-CC-020，**修订草案·待会签，未会签单边定稿无效**） | §8 配额表重构（甲方直裁 v2，2026-08-29 23:36，§COM-01）：① 废 publish/fanout 类「每日 3 次」日配额（不设每日总量上限）；② 新增短窗频控行（publish/fanout 类发布 ≤2 条/5 分钟/源，提案默认·节点本地可调·会签钉死；定位=防突发刷屏的反滥用闸，非正常行为约束）；③ 超限处置=静默丢弃超出部分+本地日志（可选告警），明示「不自动拉黑」（T3 §6.2 对应行同单标废止注记）；④ count 类（引导期计数阀门）维持既有独立低频限额（单独一行，非创作行为）。既有节最小对齐：§3 表 count/publish 行「配额计入」列、§3.1.5 配额注、§4.2 配额扣减条、§10 T1-5 行。节点侧（02 篇 FR11 / TD-18）会签后跟随 |
...
| T1-5 | 配额口径与节点 FR11 一致 | ⚠️ v0.4（修订草案·待会签）：§8 = publish/fanout 类短窗频控（≤2 条/5 分钟/源提案）+ 超限静默丢弃（不自动拉黑）+ count 类独立低频限额（每日 3 次）；节点 FR11 旧口径（publish 类每日 3 次/超限 -127）待 TD-18 会签后跟随，本行会签后复核 |
...
| v0.4 | 2026-08-29 | **KNET-CC-020 修订草案·待会签（未会签单边定稿无效）。** §8 配额表重构：publish/fanout 类日配额废除（不设每日总量上限）+ 新增短窗频控（每 5 分钟 ≤2 条/源，提案默认·节点本地可调·会签钉死；定位=防突发刷屏反滥用闸，非正常行为约束）+ 超限处置改「静默丢弃超出部分+本地日志（可选告警），不自动拉黑」（联动 T3 §6.2「配额超限自动拉黑」行同单标废止注记）+ count 类维持既有独立低频限额（每日 3 次，`dns_flood_quota_daily=3`，引导期阀门，非创作行为）。既有节最小对齐：§3 表 count/publish「配额计入」列、§3.1.5 配额注、§4.2 配额扣减条、§10 T1-5 行；头部 Version 0.3→0.4（修订草案）/Status 草案子句 + §0 修订行 + 页脚状态戳同步。依据 = 甲方直裁 v2（2026-08-29 23:36，§COM-01）+ 2026-08-28 限频定位裁定；节点侧（02 篇 FR11 / TD-18）会签后跟随 |

---

*本文件为 T1 泛洪协议 v0.3（已会签定稿：KNET-CC-007〔v0.1，2026-08-08〕+ KNET-CC-015〔v0.2，2026-08-28 节点 PM 会签通过 + 合并 master 3c45a68 联合关闭〕+ KNET-CC-019〔v0.3，2026-08-29 节点 PM 会签通过 + 协议 PM 合并 master 1dcf495 联合关闭〕），是 T3/T4/T6/T8 的共同承载层。签名覆盖范围引用 T9（`DECISIONS.md` §9.3.2）不重定义；多跳语境下 `forward_chain` 验签输入取签发前缀（§5.3；`answer` 类型例外见 §5.3-4）。v0.4（§8 配额表重构：发布类日配额废除→短窗频控、超限静默丢弃不自动拉黑、count 类维持独立低频限额）为 **KNET-CC-020 修订草案·待会签，未会签单边定稿无效**（依据 = §COM-01 2026-08-29 甲方直裁 v2；节点侧 02 篇 FR11 / TD-18 会签后跟随）。*
