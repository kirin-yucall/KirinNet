...
> **关联编号:** T1（解锁闸）/ FR6（好友链泛洪解析）/ FR11（防滥用配额）/ 9.3.2（签名覆盖范围，**引用 T9 草案·未会签**）
> **承载关系:** T1 信封是 T3（信任权重携带）/T4（多候选应答承载）/T6（好友链泛洪解析）/T8（count 包语义）的共同承载层，须最先交付。
> **签名口径:** 本文件中 `nonce` 与 `signature` 的覆盖范围**引用 T9（`DECISIONS.md` §9.3.2，P-ARCH 技术定稿口径·草案·待 KNET-CC 会签）**，P-FLOOD 引用不重定义；算法本身归 P-ARCH 主笔。
...
| 会签要求 | T1 为强制会签项，须经节点 PM 会签（KNET-CC）后方可标「定稿」；未会签前一切引用本文件的下游草案（T3/T4/T6/T8）须沿用本草案字段并标注「引用 T1 草案·未会签」 |
...
`forward_chain` **MUST 纳入签名覆盖范围**（T9 §9.3.2 第 4 项），防转发链篡改/注入。**P-FLOOD 引用 T9 草案·未会签·单边定稿无效**——签名算法、规范化规则、覆盖字段顺序归 P-ARCH 定稿，本文件不重定义。
...
> **本节为引用层，不重定义签名覆盖范围。** 签名算法、挑战码结构、覆盖字段、规范化规则、HPKE 分工全部归 T9（`DECISIONS.md` §9.3 + `did-dns-protocol.md` §3.5 + `im_protocol.md` §7.4 + `security_model_v1.md` §7.2.1），**引用 T9 草案·未会签·单边定稿无效**。
...
| T1-4 | 签名覆盖范围引用 T9 不重定义 | ✅ §7 全节标注「引用 T9 草案·未会签·单边定稿无效」；算法/覆盖字段/规范化均归 P-ARCH |
...
| 项 | 归属 | 状态 |
|---|---|---|
| 签名覆盖范围（算法/字段/规范化） | T9 / P-ARCH | **引用 T9 草案·未会签**（KNET-CC 待立单） |
