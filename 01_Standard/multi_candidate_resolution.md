...
> **Version:** 0.1（草案）
> **Date:** 2026-08-08
> **Status:** **已会签定稿（KNET-CC-009，2026-08-08 节点 PM 会签通过）**
> **Scope:** 候选 schema、权重携带、排序策略、客户端选择策略、direct/relay 候选类型与回退
> **关联编号:** T4（多候选解析格式）/ T1（answer 包承载，**引用 T1·已会签定稿（KNET-CC-007）**）/ T3（trust_weight 消费，**引用 T3·已会签定稿（KNET-CC-008）**）/ T9（候选权重字段在签名覆盖内，**引用 T9·已会签定稿（KNET-CC-011）**）/ FR7（多候选排序 `(trust_weight DESC, hops ASC, TTL DESC)`）/ FR8（多候选与冲突·direct/relay·不做唯一性裁决）
...
| 会签记录 | T4 已经节点 PM 会签（**KNET-CC-009，2026-08-08 23:45 通过**）定稿 |
...
- **引用 T9·已会签定稿（KNET-CC-011，2026-08-08）**，P-FLOOD 不重定义签名覆盖。
...
| 项 | 归属 | 状态 |
|---|---|---|
| answer 包 wire（candidates 承载） | T1 / P-FLOOD | **引用 T1·已会签定稿（KNET-CC-007，2026-08-08）** |
| trust_weight 取值与聚合 | T3 / P-FLOOD | **引用 T3·已会签定稿（KNET-CC-008，2026-08-08）** |
| 候选权重签名覆盖 | T9 / P-ARCH | **引用 T9·已会签定稿（KNET-CC-011，2026-08-08）** |
| relay tunnel_id wire 编码 | 已在 relay_protocol.md 定型（KNET-CC-013，2026-08-24 会签） | 本文件不定型，引用不重定义 |
| 客户端默认策略（取首位/direct优先） | T4 会签定稿 | **已随 KNET-CC-009 会签通过（2026-08-08）定稿** |
...
| 版本 | 日期 | 变更 |
|---|---|---|
| —（状态重戳） | 2026-08-24 | 状态滞后标注修正（非技术变更，字段/语义/条款未动）：文件头「草案 v0.1 · 未会签 · 单边定稿无效」重戳为「已会签定稿（KNET-CC-009，2026-08-08 节点 PM 会签通过）」；§0 会签要求行改会签记录（CC-009 2026-08-08 23:45 通过）、§6.3/§8 引用 T1/T3/T9 的「草案·未会签」标注改「已会签定稿（KNET-CC-007/008/011）」、§8 待会签/待定稿项改会签状态与遗留项（客户端默认策略随 CC-009 定稿）、页脚会签子句同步。波0 会签事实（master 已含），v0.1 内容不变。 |
...
*本文件为 T4 多候选解析格式，已会签定稿（KNET-CC-009，2026-08-08 节点 PM 会签通过）。候选 schema 引用 T1 answer 包、权重引用 T3、签名覆盖引用 T9，均不重定义；relay tunnel 编码已在 relay_protocol.md 定型（KNET-CC-013），引用不重定义。*
