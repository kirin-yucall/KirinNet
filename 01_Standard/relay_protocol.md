...
> **Version:** 0.1（草案）
> **Date:** 2026-08-24
> **Status:** **已会签（KNET-CC-013，2026-08-24 节点 PM 会签附条件通过·随单条件已履约·已合并 master）· 待云主机 V1~V10 联合验证通过后联合关闭**
...
| 项 | 归属 | 状态 |
|---|---|---|
| 本协议整体（relay 候选通告/tunnel_id/隧道协议/数据面基线） | KNET-CC-013 节点 PM 会签 + 波 1 云主机联合验证 | **已会签（KNET-CC-013，2026-08-24 附条件通过·履约·合并 master）· 待云主机 V1~V10 联合验证后关闭** |
...
*本文件为 KirinNet 内网穿透/中继协议 v0.1（KNET-CC-013）。已经节点 PM 会签（2026-08-24 22:30 附条件通过·随单条件履约·合并 master）；待波 1 云主机（8.133.174.128 / kirin.yucalls.com）V1~V10 联合验证通过后联合关闭并标「定稿」。候选 schema/排序/回退引用 T4、签名质询引用 T9、answer 承载引用 T1、权重引用 T3、解析层引用 T6，均不重定义；节点侧实现已随会签解锁、按 V1~V10 清单推进；打洞与 L4 透传占位不定型。*
