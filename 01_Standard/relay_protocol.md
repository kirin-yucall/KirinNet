...
> **Version:** 0.2（已会签定稿）
> **Date:** 2026-08-28
> **Status:** **v0.1 基线已会签定稿（KNET-CC-013，2026-08-26 联合关闭：V1~V10 实机全 PASS + 公网在线；2026-08-24 节点 PM 会签附条件通过·随单条件已履约·已合并 master）；v0.2 修订（§6.3 帧编码）KNET-CC-018 已会签定稿（2026-08-29 节点 PM 会签通过 + 协议 PM 合并 master 1b78ce7 联合关闭，草案 2026-08-28）**
> **注：** v0.2 为 KNET-CC-018 修订——仅涉 §6.3（含新增 §6.3.1）/§11（增 V11）/§12（增 RELAY-11）/§13/§14 与本头部，修 TD-01/TD-15（§13 预留「实机验证后微调」窗口兑现）；语义字段（t/id/method/path/status）与 §5 握手/§7 保活零改动；已会签定稿——2026-08-29 节点 PM 会签通过 + 协议 PM 合并 master 1b78ce7 联合关闭。
...
| v0.2 修订 | **KNET-CC-018 已会签定稿（2026-08-29 节点 PM 会签通过 + 协议 PM 合并 master 1b78ce7 联合关闭）**（草案 2026-08-28，P-ARCH/P-NET 视角·协议 PM 委派）：§6.3.1 体帧序列/32 KiB 分块/转发路径 MUST NOT 消费体/串行槽无进展上限 T=65s（会签钉死）+ §11 V11 + §13 处置状态——修 TD-01/TD-15；语义字段与 §5/§7 零改动 |
...
WSS 文本帧承载 JSON 控制、二进制帧承载载荷体（**体帧序列、单帧上限与转发路径约束自 v0.2 定型于 §6.3.1**——KNET-CC-018 已会签定稿（2026-08-29 节点 PM 会签通过 + 协议 PM 合并 master 1b78ce7 联合关闭））；**v1 允许串行处理**（一时刻一请求，14 篇 §7.4），但帧内 `id` 字段自 v0.1 即存在（前向兼容多路复用，多路复用本身归阶段二 R05，本协议不定型）：
...
#### 6.3.1 体帧序列、分块与转发路径约束（v0.2 修订：KNET-CC-018 已会签定稿，2026-08-29 节点 PM 会签通过 + 协议 PM 合并 master 1b78ce7 联合关闭）

> **状态：已会签定稿（KNET-CC-018，2026-08-28 P-ARCH/P-NET 视角起草；2026-08-29 节点 PM 会签通过 + 协议 PM 合并 master 1b78ce7 联合关闭）。**
...
| V11 | 大块与体帧回归（§6.3.1 v0.2，KNET-CC-018 已会签定稿，2026-08-29 节点 PM 会签通过 + 协议 PM 合并 master 1b78ce7 联合关闭）：① >32KB 体分块往返（样本建议 ≥1MB，含非整块余数与 0 体边界）；② 经隧道反代的 `/api/*` POST 体帧端到端（登录/IM/内容发布族任一真实接口）；③ 违例注入：超限单帧 / 体长不符 content-length → `frame_invalid` + 连接不断 + 槽位即释；④ 无进展上限 T 到期 → `local_unreachable` 收尾不悬挂 | 分块重组字节一致 + /api POST 2xx + 注入用例连接存活/槽位即释 + T 用例在量级内失败收尾 | ✅ 节点侧已执行（N-WS-RELAY 4 commit 三级验收 PASS + 独立复跑 138/138，四用例直证，合并节点 main 2b051ca，2026-08-29） |
...
| RELAY-11 | v0.2 修订边界自查（KNET-CC-018 已会签定稿，2026-08-29 节点 PM 会签通过 + 协议 PM 合并 master 1b78ce7 联合关闭）：语义字段（t/id/method/path/status）与 §5 握手/§7 保活零改动（diff 核验）；与 §6.1/§6.2/§6.4/§9.3 零矛盾；引用节号（§5.1/§6.2/§7.1/§9.3/§9.4）核验；RELAY-9 检查口径复验（残留仍仅本表命令文本） | ✅ |
...
| 隧道帧编码细节（header 折叠/分块边界） | 波 1 实机验证后微调（语义字段已定型）→ 分块边界归 KNET-CC-018（v0.2）处置 | **已闭环（KNET-CC-018 已会签定稿，2026-08-29 节点 PM 会签通过 + 协议 PM 合并 master 1b78ce7 联合关闭，草案 2026-08-28）**：分块边界已定型 §6.3.1（体帧序列全路径 + ≤32 KiB 分块 + 转发不消费体 + 楔死防线）——TD-01 随单闭环（节点侧 N-WS-RELAY 4 commit 三级验收 PASS、独立复跑 138/138、V11 四用例直证、合并节点 main 2b051ca）；header 折叠暂无修订需求、继续挂起；语义字段零改动 |
| 体帧缺失致隧道反代 /api POST 体丢失（TD-15） | KNET-CC-018（实现侧 N-WS-RELAY 随单：relay_server.js 体帧补发 + 流式化） | **已闭环（KNET-CC-018 已会签定稿，2026-08-29 节点 PM 会签通过 + 协议 PM 合并 master 1b78ce7 联合关闭，草案 2026-08-28）**：§6.3.1（a）体帧序列适用全部路径（含 `/api/*`）+（d）转发路径 MUST NOT 消费载荷体；TD-15 随单闭环（节点侧 N-WS-RELAY 4 commit 三级验收 PASS、独立复跑 138/138、V11 四用例直证、合并节点 main 2b051ca；V11-② 端到端联测 PASS） |
...
| v0.2 | 2026-08-28 | **KNET-CC-018 已会签定稿（2026-08-29 节点 PM 会签通过 + 协议 PM 合并 master 1b78ce7 联合关闭）**（草案 2026-08-28，P-ARCH/P-NET 视角·协议 PM 委派）：§6.3 帧编码修订，兑现 §13 预留「实机验证后微调」窗口，修 TD-01/TD-15——**新增 §6.3.1**：体帧序列（req/res→体帧 ×0..n→end；`id`+顺序即边界、无序号字段；适用全部路径含 `/api/*`；零体 0 帧合法；content-length 一致性 fail-closed）+ 单帧 ≤32 KiB 分块互操作上限（接收端 MUST 接受 ≤32 KiB、超限拒收）+ 超限/违例 `frame_invalid` 复用且 non-fatal（连接不断、槽位即释）+ 串行槽无进展上限 T=65s（提案，会签钉死；`local_unreachable` 复用收尾）+ 转发路径 MUST NOT 消费载荷体（控制面解析限定 §5.1 端点，实现指引不规定实现）；§6.3 表两行「二进制体块」→「二进制体帧 ×0..n」+ 尾段预留窗口兑现注记；§11 增 V11（大块分块往返 + /api POST 体帧端到端 + 违例注入 + T 超时）；§12 增 RELAY-11；§13 两行处置中；头部 Version 0.1→0.2 + 草案注 + §0 增 v0.2 修订行。**语义字段（t/id/method/path/status）与 §5 握手/§7 保活零改动**（KNET-CC-013 定稿口径不变） |

---

*本文件为 KirinNet 内网穿透/中继协议 v0.2（已会签定稿）。v0.1 基线已会签定稿（KNET-CC-013）：节点 PM 会签（2026-08-24 22:30 附条件通过·随单条件履约·合并 master）→ 云主机（8.133.174.128 / kirin.yucalls.com）V1~V10 实机验证全项 PASS + 公网在线 → 联合关闭（2026-08-26）。候选 schema/排序/回退引用 T4、签名质询引用 T9、answer 承载引用 T1、权重引用 T3、解析层引用 T6，均不重定义；节点侧实现已随会签解锁、按 V1~V10 清单推进；打洞与 L4 透传占位不定型。v0.2 修订（KNET-CC-018 已会签定稿，2026-08-29 节点 PM 会签通过 + 协议 PM 合并 master 1b78ce7 联合关闭，草案 2026-08-28）：§6.3.1 体帧序列/32 KiB 分块/转发路径不可消费 + §11 V11，修 TD-01/TD-15；语义字段与 §5/§7 零改动。*
