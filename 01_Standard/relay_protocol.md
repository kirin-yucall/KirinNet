...
| 会签要求 | 强制会签项，须经节点 PM 会签（KNET-CC-013）后方可标「定稿」；波 1 云主机（8.133.174.128）联合验证通过是定稿前置条件 |
...
- **服务名先例**：`_kirinnet-ipfs._tcp` 已预留（spec_v1 §2.3 迁移注记 / dns_automation §9）、`_kirinnet-quic._udp` 未来扩展（spec_v1 §5.3）——`_kirinnet-relay._tcp` 同型扩展。**定稿时须随会签在 spec_v1 §2.2 服务名表 + §6.1 IANA 表各加一行**（KNET-CC-013 修订范围）。
...
- **签名覆盖**：候选 `tunnel_id` 字段是否纳入 T9 §9.3.2 签名覆盖 → **待会签钉死**（§13 观察点：篡改 tunnel_id 无安全后果——非凭证非路由键——建议不扩覆盖，维持 T9 现清单）。
...
命名说明：`/api/v2/*` 命名空间与节点仓 api.md v2.x 版本线对齐；**与 14 篇 §5.1 `/api/relay/*`（节点本地管理面，status/connect/config）是不同层**——那是节点自身 admin API，归节点仓定义，本协议不碰。
...
> **基线来源：** 以节点 14 篇规划为输入提炼（FR2 Host/SNI 路由 + §7.1「v1 走 WSS 零新依赖」+ §7.4「v1 可退化为串行转发」）。**节点侧实现（R01/R02）待会签后启动 fail-closed**（同 T6 §7 模式）。
...
**注意：** spec_v1 §4.1 缓解措施 3「证书 SAN MUST 匹配原域名而非 SRV target」按字面适用于 direct 场景；relay 场景下传输对端是 relay，须按本节分层。**定稿时随会签在 spec_v1 §4.1 加一行 relay 场景注记**（KNET-CC-013 修订范围，不改 direct 语义）。
...
| spec_v1（ADRP） | §3.1/§3.2 使用 SRV 既有语义；定稿随附两处一行修订（§2.2/§6.1 加 `_kirinnet-relay._tcp`；§4.1 relay 注记） |
| 节点 14 篇 | 本协议的规划输入；节点侧 R01~R05 实现**待会签后启动 fail-closed** |
...
| RELAY-8 | 节点侧实现标注「待会签后启动 fail-closed」 | ✅ §6/§0 |
...
| 项 | 归属 | 状态 |
|---|---|---|
| 本协议整体（relay 候选通告/tunnel_id/隧道协议/数据面基线） | KNET-CC-013 节点 PM 会签 + 波 1 云主机联合验证 | **草案 v0.1** |
| spec_v1 §2.2/§6.1 追加 `_kirinnet-relay._tcp` 服务名（各一行） | 随 CC-013 会签携带 | 草案 |
| spec_v1 §4.1 缓解措施 3 的 relay 场景注记（一行） | 随 CC-013 会签携带 | 草案 |
| T4 §8 占位行回填指向 relay_protocol.md（一行，含「T7」编号歧义澄清） | 随 CC-013 会签携带 | 草案 |
| 候选 `tunnel_id` 是否纳入 T9 签名覆盖 | 会签观察点（建议：不扩，维持 T9 现清单——tunnel_id 非凭证非路由键） | 待钉死 |
| 隧道帧编码细节（header 折叠/分块边界） | 波 1 实机验证后微调（语义字段已定型） | 待 V1~V10 |
