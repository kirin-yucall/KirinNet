...
### 3.5 签名质询-应答验证流程（T9 · 草案·待 KNET-CC 会签）
...
5. **T3 权重字段**（`weight`/`trust_score`）—— P-FLOOD T3 草案字段（防权重篡改）。
...
> **会签要求：** T9 须经节点 PM 会签（KNET-CC，影响节点侧签名/验签代码）后方可标「定稿」。P-FLOOD T1/T3 草案引用本节签名覆盖范围时须标注「引用 T9 草案·未会签·单边定稿无效」。
