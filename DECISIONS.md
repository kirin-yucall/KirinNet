...
| 已知滞后（记录不代改） | `flood_protocol.md` / `content_query.md` / `relay_protocol.md` 三文件头状态字样滞后于台账（CC-015/014/013 均已联合关闭），已记 `BASELINE.md` §2；**批 4（2026-08-28 验收反馈补件）已随本分支重戳闭环**——只动状态行/§0 状态子句/会签状态表/页脚，条款零改动（同观察点 012-⑥ 先例） |
